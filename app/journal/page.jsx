      ...
      <div className="space-y-4">
        {logs.map((log) => (
          ...
          >
            ...
            <p className="text-gray-300 mb-2">{log.rawNotes || 'No raw notes provided.'}</p>
            {log.planetName && (
              <span className="text-xs bg-gray-800 px-2 py-1 rounded text-gray-400">Planet: {log.planetName}</span>
            )}
            {log.aiNarrative && (
              <p className="text-sm text-gray-400 mt-2 border-t border-gray-800 pt-2">
                AI Narrative: {log.aiNarrative}
              </p>
            )}
          </div>
        ))}
