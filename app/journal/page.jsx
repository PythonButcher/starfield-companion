            ...
            <div className="space-y-4">
                {logs.map((log) => (
                    <div key={log.id} className="border border-gray-700 p-4 rounded hover:border-hud-blue transition-colors bg-gray-900/50">
                        ...
                        <p className="text-gray-300 mb-2">{log.raw_notes || log.ai_narrative || 'No notes recorded.'}</p>
                        {log.planet_name && <span className="text-xs bg-gray-800 px-2 py-1 rounded text-gray-400">Planet: {log.planet_name}</span>}
                    </div>
                ))}
            </div>
        </div>
